...
def cal_dir_avg_start(x,type,n_chose=20):
    #n_chose=10
    if type=='end':
        work=x
    else:
    ...
    dir_ans=np.array([0.0,0.0])
    for i in range(1,len(work)):
        dir_ans+=((work[i][0]-work[i-1][0],work[i][1]-work[i-1][1]))
    dir_ans=dir_ans/np.linalg.norm(dir_ans) 
...
def cal_jump_cost(dir_s,p_s,dir_e,p_e):
    ...
    if (inner_product(dir_m,dir_s)<0 or inner_product(dir_e,dir_m)<0) and cal_dis(p_s,p_e)>=50:
        return(pinf)
    else:
        #print(cal_dis(dir_s,dir_m))
...
#        print(cal_dis(p_s,p_e))
#        print('------------')
#        print ([ag1,ag2,cal_dis(p_s,p_e)])
#        print ([ag1/180,ag2/180,cal_dis(p_s,p_e)/100])
        
        val=-(ag1/180+ag2/180+cal_dis(p_s,p_e)/150)
...
def scatterindex(x_list,state_all):
    ...
    for i,xx in enumerate(x_list):
        ...
        if xx%2==1:
            temp=temp[::-1]
        x=np.array(temp)    
        plt.scatter(x[:,0],x[:,1],zorder=1,c='b')
        if i==0:
            plt.scatter(x[:,0],x[:,1],1,label='start',zorder=2,c='r')
        if i==len(x_list)-1:    
            plt.scatter(x[:,0],x[:,1],1,label='end',zorder=2,c='y')
        if i>0:
...
def scatterindexsp(x_list,state_all,color):
    #plt.figure()
    st=[-1,-1]
    for i,xx in enumerate(x_list):
        temp=state_all[xx//2]
        if xx%2==1:
            temp=temp[::-1]
        x=np.array(temp)    
        plt.scatter(x[:,0],x[:,1],zorder=1,c=color)
        #if i==0:
        #    plt.scatter(x[:,0],x[:,1],1,label='start',zorder=2,c='r')
        #if i==len(x_list)-1:    
        #    plt.scatter(x[:,0],x[:,1],1,label='end',zorder=2,c='y')
...
def same_ellipse2(el1,el2):
    ...
    # print(norm(el1_0-el2_0)/(0.5*(norm(el1_0)+norm(el2_0))))
    # print(norm(el1_1-el2_1)/(0.5*(norm(el1_1)+norm(el2_1))))
    if norm(el1_0-el2_0)/(0.5*(norm(el1_0)+norm(el2_0)))<0.05 and \
       norm(el1_1-el2_1)/(0.5*(norm(el1_1)+norm(el2_1)))<0.1:
           return(True)
    else:
        return(False)
    
def same_ellipse3(el1,el2):
    el1_0=np.array(el1[0])
    el2_0=np.array(el2[0])
    el1_1=np.array(el1[1])
    el2_1=np.array(el2[1])
    ...
    # print(norm(el1_0-el2_0)/(0.5*(norm(el1_0)+norm(el2_0))))
    # print(norm(el1_1-el2_1)/(0.5*(norm(el1_1)+norm(el2_1))))
    if norm(el1_0-el2_0)/(0.5*(norm(el1_0)+norm(el2_0)))<0.1 and \
       norm(el1_1-el2_1)/(0.5*(norm(el1_1)+norm(el2_1)))<0.3:
           return(True)
    else:
        return(False)
