...
class surf(_CocoStuff):
    """COCO-Stuff 164k dataset"""

    def __init__(self, **kwargs):
        super(surf, self).__init__(**kwargs)
        #self.ignore_label = 255

    def _set_files(self):
        # Create data list by parsing the "images" folder
        if self.split in ["train", "test"]:
            fileindexnames = [x.strip('\n') for x in open(osp.join(self.root,self.split)+".txt", 'r')]
            file_list=[]
            for indexname in fileindexnames:
                  file_list=file_list+(glob.glob(self.root+'/'+str(indexname)+'*_data.png'))
    ...
    def _load_data(self, image_id):
        # Set paths
        image_path = osp.join(self.root, image_id + ".png")
        label_path = osp.join(self.root, image_id[:-4]+'true.png')
        # Load an image
        image = cv2.imread(image_path, cv2.IMREAD_COLOR).astype(np.float32)
        label = cv2.imread(label_path, cv2.IMREAD_GRAYSCALE).astype(np.uint8)
        label =(label//99).astype(np.uint8)
        return image, label
...
if __name__ == "__main__":
    ...
    dataset_root = "../../data/cir"
    dataset = surf(root=dataset_root, split="train")
    ...
    for i, (images, labels) in tqdm(
        ...
        if i == 0:
            plt.figure
            plt.imshow((np.squeeze(labels[1,:,:].numpy().astype(np.uint8))))
            plt.figure
            plt.imshow(np.transpose(np.squeeze(images[1,:,:,:].numpy().astype(np.uint8)),(1,2,0)))
            mean = (
                torch.tensor((134.5, 134.5, 134.5))
                .unsqueeze(0)
                .unsqueeze(2)
                .unsqueeze(3)
            )
